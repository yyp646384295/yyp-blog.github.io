...
var getPropsReg1 = /^<preview (path|title|description)=(.*) (path|title|description)=(.*) (path|title|description)=(.*)><\/preview>$/;
var getPropsReg2 = /^<preview (path|title|description)=(.*) (path|title|description)=(.*) (path|title|description)=(.*) \/>$/;
/**
 * 编译预览组件
 * @param md
 * @param token
...
var transformPreview = function (md, token, env) {
    ...
    // 获取Props相关参数
    var tokenContentArr = token.content.match(getPropsReg1) || token.content.match(getPropsReg2) || [];
    tokenContentArr.shift();
    for (var i = 0; i < tokenContentArr.length; i += 2) {
        var item = tokenContentArr[i];
        if (item === 'path') {
